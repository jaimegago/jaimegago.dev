...
draft: false
project_link: >-
  **Joe** → [joeagent.dev](https://joeagent.dev) · [source](https://github.com/jaimegago/joe) · [safety deep dive](https://joeagent.dev/safety/)
lead: >-
...
{{< figure src="images/joe-launch/flyball-governor.png" float="right" floatwidth="35" alt="An engraved-style illustration of a brass flyball governor on a wooden base" caption=`A centrifugal "flyball" governor — the first machine built to govern a machine. Joe wears it as its mark.` >}}
...
{{< figure src="images/joe-launch/teams-ask-redacted.png" float="left" alt="Teams message asking the platform lead to run kubectl get mutatingwebhookconfigurations on the cluster" caption="The handoff: my theory, and the one command my permissions couldn't run." >}}

He checked. Minutes later: *"you are righ about the mutating webhook!!!!"* (typo his, enthusiasm preserved).

{{< figure src="images/joe-launch/teams-confirmation-redacted.png" float="right" alt="Teams reply confirming the mutating webhook was the culprit" caption="The payoff, verbatim." >}}
...
There's nearer-term work too: the agent harness itself. Joe is the first AI agent I've ever built, and the machinery around the model — assembling context, pruning history, budgeting tokens, turning tool errors into signals the model can act on — is where an agent's quality is actually decided. Mine is young. So is my grasp of the craft — building Joe taught me enough to know how much of agent engineering I haven't touched yet. Recording the launch demo made that plain: the model repeatedly proved smarter than the harness I'd wrapped around it. Which is also why I'm optimistic — the model isn't the bottleneck, my code is, and my code is exactly what real-world use improves fastest.
